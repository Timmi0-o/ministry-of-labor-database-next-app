...
export const ParkPc = () => {
	...
	return (
		<Section>
			{/* TITLE  */}
			<h1 className='lg:text-[2vw] font-bold tracking-[0.1vw] mt-[0.3vw] mb-[2vw] text-center w-full'>
				Сотрудники
			</h1>
			{/* LOADING  */}
			<div
			...
			{/* LIST PC */}
			<div
				className={`flex flex-wrap gap-[1vw] size-full ease-out duration-300 ${
					data ? '' : 'ml-[100%] opacity-0 fixed'
				}`}
			>
				{data?.map((user: User, i: number) => (
					<div
						...
						className={`relative flex items-center mb-[1vw] duration-300 h-fit`}
						key={i}
					>
						<div
						...
						>
							...
							{user.pc &&
								...
								user.pc.map((pc, j) => (
									<div
										className='relative flex flex-col gap-[0.4vw]'
										key={pc.registrationNumber}
									>
										<div className='size-[1.3vw] mt-[1vw]'>
							...
							<div
								className={`flex gap-[0.1vw] w-full duration-200 ease-in-out mt-[1vw] ${
									isItemHovered === i
										? ''
										: 'opacity-0 select-none translate-y-[0.5vw]'
								}`}
							>
								{/* DELETE FULL USER  */}
								<div
			...
			{data?.length === 0 && (
				<div className='w-full flex justify-center'>
					<h2 className='text-[2vw] w-[80%] text-center text-[#373737] font-medium bg-[#E1F2F7] rounded-[0.2vw]'>
						Тут пока пусто...
					</h2>
				</div>
			)}
